...
impl SDRThread {
    fn new(config: SDRConfig, spectrum_tx: Sender<SpectrumData>) -> Self {
        let fft = Radix4::new(config.fft_size, false);
    ...
    fn generate_simulated_iq(&self, buffer: &mut [Complex32]) {
        ...
        for (i, sample) in buffer.iter_mut().enumerate() {
            ...
            let q = (2.0 * std::f32::consts::PI * center_freq * t + std::f32::consts::PI / 2.0).sin() 
                   + (2.0 * std::f32::consts::PI * (center_freq + 10000.0) * t + std::f32::consts::PI / 2.0).sin() * 0.5
                   + noise;
